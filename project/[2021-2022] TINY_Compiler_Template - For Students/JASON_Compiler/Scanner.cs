...
public enum Token_Class
{
    ...
    Dot, Semicolon, Comma, LParanthesis, RParanthesis, EqualOp, EqualConOp, LessThanOp,
    ...
    public class Scanner
    {
       ...
       public bool fromOperation = false;
        ...
        public Scanner()
        {
            ...
            ReservedWords.Add("main", Token_Class.Identifier);
            ...
            else 
            {
                if(fromOperation)
                {
                    ...
                    while (true)
                    {
                        ...
                        if (Lex.IndexOf(":=") >= 0)
                        {
                            substring = Lex.Substring(Lex.IndexOf(":="), 2);
                            Lex = Lex.Remove(Lex.IndexOf(":="), 2);
                            FindTokenClass(substring);
                        }
                        else if (Lex.IndexOf("=") >= 0)
                        {
                            substring = Lex.Substring(Lex.IndexOf("="), 1);
                            Lex = Lex.Remove(Lex.IndexOf("="), 1);
                            FindTokenClass(substring);
                        }
                        else if (Lex.IndexOf(">") >= 0)
                        {
